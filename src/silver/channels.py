...
class Channels:
    ...
    def _process_silver_latest(self) -> None:
        ...
        if self.file_path_silver_latest.exists():
            ...
            if len(df_insert) > 0:
                df_insert_cleaned = df_insert.drop(columns=['_checksum_silver_latest']).rename(columns={'_checksum_bronze': '_checksum'})
            ...
            if len(df_update) > 0:
                df_update_cleaned = df_update.drop(columns=['_checksum_silver_latest']).rename(columns={'_checksum_bronze': '_checksum'})
